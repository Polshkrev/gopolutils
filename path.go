...
func NewPath() *Path {
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, NewNamedException("OSError", err.Error()))
		os.Exit(1)
	}
...
func PathFromParts(folderName, fileName, fileType string) *Path {
	...
	if absoluteError != nil {
		fmt.Fprintln(os.Stderr, NewNamedException("OSError", absoluteError.Error()))
		os.Exit(1)
	}
...
func (path Path) Absolute() (*Path, *Exception) {
	var absolute string
	var absoluteError error
	absolute, absoluteError = filepath.Abs(path.raw)
	if absoluteError != nil {
		return nil, NewNamedException("OSError", absoluteError.Error())
	}
...
func (path Path) Suffix() (string, *Exception) {
	var index int = strings.LastIndexByte(path.raw, '.')
	if index < 0 {
		return "", NewNamedException("OSError", "Path does not have an associated suffix.")
	}
...
func getRoot(filePath string) (string, *Exception) {
	var index int = strings.IndexRune(filePath, filepath.Separator)
	if index < 0 {
		return "", NewNamedException("OSError", "Path does not have an associated root.")
	}
...
func (path Path) Root() (*Path, *Exception) {
	var absolute string
	var absoluteError error
	absolute, absoluteError = filepath.Abs(path.raw)
	if absoluteError != nil {
...
func checkRoot(path Path) *Exception {
	var root *Path
	var rootExcept *Exception
	...
	if rootExcept != nil {
		...
		return NewNamedException("OSError", "Can not get parent of filesystem root.")
	}
	return nil
}
...
func (path Path) Parent() (*Path, *Exception) {
	var rootExcept *Exception = checkRoot(path)
	if rootExcept != nil {
		return nil, rootExcept
	}
