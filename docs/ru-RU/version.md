...
### Сравнивать
Каждое из числовых свойств версии имеет метод сравнения. Чтобы сравнить с другой одной версии доступен метод `Compare`. Эти методы сравнения определяют, больше ли числовые свойства версии или равны заданному параметру сравнения. Метод `IsPublic` доступен и используется в методе `Publish`. В методе `Publish`, если версия уже является публичной, возвращается `ValueError`, а объект версии **не изменяется**. Метод `IsPublic` возвращает значение `true`, если свойство версии «major» больше или равно единице. Чтоб определить того, равна ли версия нулю, доступен метод `IsZero`.